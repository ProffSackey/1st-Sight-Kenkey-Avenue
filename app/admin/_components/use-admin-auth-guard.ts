...
export function useAdminAuthGuard(loadingFallback = 'Loading admin page...') {
  ...
  useEffect(() => {
    ...
    const guardPage = async () => {
      try {
        ...
        setFullName(employee?.fullName || 'Admin User');
  ...
  return { email, fullName, isLoading, loadingMessage };
}
