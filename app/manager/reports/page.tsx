...
interface AnalyticsState {
  ...
  topEmployee: { name: string; email: string; orders: number; revenue: number } | null;
  topItem: { name: string; quantity: number; revenue: number } | null;
  topPaymentMethod: { method: string; count: number; share: number; revenue: number } | null;
  paymentMix: { method: string; count: number; share: number; revenue: number }[];
  trend: TrendPoint[];
}
...
export default function ManagerReportsPage() {
  const { email, fullName, branch, isLoading: authLoading } = useManagerAuthGuard('Loading manager reports...');
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const employeeAgg = new Map<string, { name: string; email: string; orders: number; revenue: number }>();
        ...
        orders.forEach((order) => {
          ...
          const existingEmployee = employeeAgg.get(order.cashier_id) || {
          ...
          employeeAgg.set(order.cashier_id, {
            ...existingEmployee,
            orders: existingEmployee.orders + 1,
            revenue: existingEmployee.revenue + amount,
          });

          const existingPayment = paymentAgg.get(payment) || { count: 0, revenue: 0 };
          paymentAgg.set(payment, {
            count: existingPayment.count + 1,
            revenue: existingPayment.revenue + amount,
          });

          (order.order_items || []).forEach((oi) => {
            const quantitySold = Number(oi.quantity_sold || 0);
            const existingItem = itemAgg.get(oi.item_id) || {
              name: 'item' in oi && oi.item?.name ? oi.item.name : 'Unknown Item',
              quantity: 0,
              revenue: 0,
            };
            itemAgg.set(oi.item_id, {
              ...existingItem,
              quantity: existingItem.quantity + quantitySold,
              revenue: existingItem.revenue + Number(oi.line_total || 0),
            });
        ...
        console.error('Manager reports analytics error:', fetchError instanceof Error ? fetchError.message : String(fetchError), fetchError);
        if (active) {
          setAnalytics(emptyState);
          setError('Could not load branch analytics.');
          setLoading(false);
        }
      }
    };
  ...
  if (authLoading || loading) {
    return <main className="flex min-h-screen items-center justify-center">Loading manager reports...</main>;
  }
          ...
          <div className="relative rounded-2xl border border-slate-100 bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 p-5 text-white shadow-lg">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <h1 className="mt-2 text-2xl font-bold sm:text-3xl">Branch Performance Intelligence</h1>
                <p className="mt-2 text-sm text-sky-100/90">
                  Track revenue momentum, team impact, and payment behavior for {branch}.
                </p>
              </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-emerald-300/30 bg-emerald-400/10 p-3">
                <p className="text-[11px] uppercase tracking-wide text-emerald-200">Weekly Revenue Pulse</p>
          ...
          <div className="relative mt-6 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(0,0.95fr)]">
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <p className="text-xs uppercase tracking-wide text-slate-500">Team Activity</p>
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                    <p className="text-[11px] uppercase tracking-wide text-slate-500">Active Cashiers</p>
