...
export function ManagerShell({ children, email, fullName, branch }: ManagerShellProps) {
    ...
    <main className="min-h-screen bg-slate-100 text-slate-900">
      {isResetPasswordOpen && (
        <AccountPasswordModal
          accountLabel={fullName || 'your manager account'}
          ...
          <div className="relative shrink-0" ref={menuRef}>
            ...
            >
              ...
              <div className="hidden sm:block">
                <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Manager</p>
      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-0 pt-0 pb-6">
        ...
        >
          <div className="flex h-full min-h-0 flex-col overflow-hidden px-3 py-4 lg:px-4 lg:py-5">
            <div className={`flex items-center border-b border-slate-200/80 pb-5 ${isSidebarOpen ? 'justify-between' : 'justify-center'}`}>
              {isSidebarOpen && (
                <div>
            ...
            <nav className="flex-1 overflow-y-auto pt-5">
              <div className="space-y-2 pb-14">
                {sidebarItems.map((item, index) => {
                  const isActive = item.href === '/manager' ? pathname === item.href : pathname.startsWith(item.href);
                  const isLastItem = index === sidebarItems.length - 1;
                  const sharedClasses = `group flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition-all ${
                    isActive
                  ...
                  if (!item.available) {
                    return (
                      <div key={item.label} title={!isSidebarOpen ? item.label : undefined} className={sharedClasses}>
                        <span
            ...
            {isSidebarOpen && (
              <div className="mt-auto rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-emerald-600">Assigned Branch</p>
                <p className="mt-1 text-sm font-semibold text-slate-900">{branch || 'No branch assigned'}</p>
                <p className="mt-1 text-xs text-slate-500">Manager access is scoped to your branch operations.</p>
              </div>
            )}
          </div>
        </aside>
